...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
                                     ...
                                     double[][] drywet, double[][] wetwet){
                                        ...
                                        for(int i = 0; i < wetwet.length; i++){
                                            if(wetwet[i][0] == longitude && drywet[i][1] == latitude){
                                                for(int j = 0; j < 12; j++){
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        int [] generatedForecast = new int [numberOfDays];

        int b = 0;
        if(a < .5){
            generatedForecast[0] = 1;
            b = 1;
        } else {
            generatedForecast[0] = 2;
            b = 2;
        }
        ...
        for (int i = 1; i < numberOfDays; i++){
            ...
            if(b == 1){
                if(randomNumber <= wetwetProbability){
                    generatedForecast[i] = 1;
                    b = 1;
                } else {
                    generatedForecast[i] = 2;
                    b = 2;
                }
            } else {
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for(int i = 0; i < forecast.length-6; i++){
            a = forecast[i] + forecast[i+1] + forecast[i+2] + forecast[i+3] + forecast[i+4] + forecast[i+5] + forecast[i+6];
                if(a > b){
                    c = i;
                    b = a;
                }
