public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int indexLeaseDuration = (description.indexOf("length:") + 7); 
        String leaseDurationTest = description.substring(indexLeaseDuration); 
        int indexLeaseDurationEnd = (leaseDurationTest.indexOf(';')); 
        String leaseLengthString = leaseDurationTest.substring(0, indexLeaseDurationEnd);
        int leaseDuration = Integer.parseInt(leaseLengthString);
        ...
        Lease createLease = new Lease(dueAtSigning, leaseDuration, monthlyCost, mileageAllowance);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        double leasePrice = 0;
        double CO2 = 8.887;
        double conversion = .453;
        for ( int i = 0; i < vehicles.length; i ++ )
        {
            double leaseDuration = vehicles[i].getLease().getLeaseLength();
            double mileage = vehicles[i].getLease().getMileageAllowance() * leaseDuration;
            double milesPerUnit = vehicles[i].getFuel().getUsage();

            double unitPrice = gasPrice;
            double monthLeaseCost = vehicles[i].getLease().getMonthlyCost();
            if (vehicles[i].getFuel().getType() == 1)
            {
                emissions = mileage / milesPerUnit * CO2;
                leasePrice = (monthLeaseCost * leaseDuration)  + vehicles[i].getLease().getDueAtSigning();
            }
            else if(vehicles[i].getFuel().getType() == 2)
            {
                double electricityPerCharge = vehicles[i].getFuel().getCharger();
                emissions = mileage/milesPerUnit * conversion;
                unitPrice = electricityPrice;
                leasePrice = (monthLeaseCost * leaseDuration) + vehicles[i].getLease().getDueAtSigning() + electricityPerCharge;
            }
            
            vehicles[i].setCO2Emission(emissions);
            double fuelPrice = (mileage/milesPerUnit) * unitPrice;
            vehicles[i].setFuelCost(fuelPrice);
            double totalPrice = leasePrice + fuelPrice;
            vehicles[i].setTotalCost(totalPrice);
        }
    	}
